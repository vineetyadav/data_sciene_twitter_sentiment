...
for each_tweet in tweet_file:
    ...
    if len(tweet_tokens):
        ...
        tweet_score = 0
    for each_token in tweet_tokens:
        if not sentiment_dict.get(each_token,0):
            word_score = tweet_score
        if sentiment_dict.get(each_token,0):
            word_score = sentiment_dict[each_token]
        print each_token.encode("utf-8","ignore")," ", str(float(word_score))
